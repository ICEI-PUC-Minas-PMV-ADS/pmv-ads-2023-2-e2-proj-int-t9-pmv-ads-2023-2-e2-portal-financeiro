...
namespace Portal_financeiro2023.Controllers
{
    public class UsuariosController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        private async Task<IActionResult> Edit1(int id, [Bind(new[] { "Id,Nome,Senha,Perfil" })] Usuario usuario)
        {
            if (id != usuario.Id)
            {
                return NotFound();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        private async Task<IActionResult> Create1([Bind(new[] { "Id,Nome,Senha,Perfil" })] Usuario usuario)
        {
            if (ModelState.IsValid)
            {
                usuario.Senha = BCrypt.Net.BCrypt.HashPassword(usuario.Senha);
